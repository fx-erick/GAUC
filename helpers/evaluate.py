...
label_crc100k = ['TUM', 'STR', 'NORM', 'MUC', 'DEB', 'ADI', 'LYM', 'MUS']
...
def confusion_matrix(
    ...
    *,
    ...
    metric = ConfusionMatrix(
        task=task, num_classes=len(encode_label_crc100k), normalize=normalize
    )
    
    sorted_labels = sorted(label_crc100k, reverse=True)
...
def process_df(df, answer_to_label):
    df["full_response"] = df["full_response"].apply(lambda x: json.loads(x) if isinstance(x, str) else x)  # convert from jsonstring to dict
    df_exploded = df["full_response"].apply(pd.Series)  # explode dict into columns
    ...
    unique_labels = set(df['label'].unique()).union(set(df['model_response'].unique()))
    print(f"Unique classes found: {unique_labels}")
